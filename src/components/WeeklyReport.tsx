...
// Retorna o início da semana (segunda-feira) e o fim (domingo)
...
export function WeeklyReport() {
  ...
  const [saleToCancelId, setSaleToCancelId] = useState<string | null>(
    null,
  );
  ...
  const weekLabel = useMemo(() => {
    return `${formatDate(monday.getTime())} a ${formatDate(
      sunday.getTime(),
    )}`;
  }, [monday, sunday]);
  ...
  const weekSales = useMemo(() => {
    return sales.filter((s) => {
      ...
      const isThisWeek =
        ts >= monday.getTime() && ts <= sunday.getTime();
  ...
  // Métricas totais das vendas filtradas
  const validWeekSales = weekSales.filter(
    (s) => s.status === 'APROVADA',
  );

  const totalPix = validWeekSales
    .filter((s) => s.method === 'PIX')
    .reduce((acc, s) => acc + s.total, 0);

  const totalCartao = validWeekSales
    .filter((s) => s.method === 'CARTAO')
    .reduce((acc, s) => acc + s.total, 0);

  const totalDinheiro = validWeekSales
  ...
  const handleCancelSale = (saleId: string) => {
    // Somente ADMIN pode iniciar o cancelamento
  ...
  // Após validar o PIN do administrador
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto pb-20">
      ...
      <div className="bg-surface p-6 rounded-2xl border border-line mb-4 text-center shadow-sm">
        <div className="text-txt-secondary mb-1 text-sm">
        ...
        <div className="text-xs text-txt-secondary mt-1">
          {validWeekSales.length} vendas aprovadas
        </div>
      </div>
      ...
      {groupedByDay.length === 0 ? (
        <div className="text-center text-txt-secondary py-16 bg-surface rounded-2xl border border-line">
          ...
          <p className="font-medium">
            Nenhuma venda encontrada para este filtro na semana
            atual.
          </p>
        </div>
      ) : (
        <div className="space-y-8">
          {groupedByDay.map((group) => {
            const groupValid = group.items.filter(
              (s) => s.status === 'APROVADA',
            );

            const groupTotal = groupValid.reduce(
              (acc, s) => acc + s.total,
              0,
            );
