...
export const DemoLockGate: React.FC<DemoLockGateProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0D1117] flex items-center justify-center p-4 text-slate-100">
      <div className="w-full max-w-md bg-[#161B22] border border-slate-800 rounded-2xl p-6 shadow-2xl text-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <input
              ...
              className="w-full bg-[#0D1117] border border-slate-800 rounded-xl px-4 py-3 text-center text-lg font-mono text-teal-400 focus:outline-none focus:border-teal-500 transition-colors"
            />
            {error && (
